...
export function makeEditor(textarea, editorTools=defaultToolbar) {
  let os = "win";
  ...
  for (let format of options.formats) {
    if (Object.keys(defaultHandlers).indexOf(format) != -1)
      options.modules.toolbar.handlers[format] = defaultHandlers[format]
  }
