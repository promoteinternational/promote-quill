...
class PromoteVideo extends BlockEmbed {
  static create(value) {
    ...
    iframe.setAttribute('sandbox', 'allow-forms allow-pointer-lock allow-scripts allow-same-origin');
    iframe.setAttribute('height', DEFAULT_ATTRIBUTES['height']);
    iframe.setAttribute('width', DEFAULT_ATTRIBUTES['width']);
    iframe.setAttribute('src', LinkParser.ensureProtocol(extractVideoUrl(value)));
